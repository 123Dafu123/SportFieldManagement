...
{
    ui->WaringL->setText("");
    ui->dataLV->setModel(MgrBLL::getStatisticFMbrFUsingModelByAll());
    ui->InfoT->close();
    ui->FinshB->show();
...
void MgrWindow::on_FinshB_clicked()
{
    if(finshModel==Add)
    {
        ...
        case IdSta:
            ...
            {
                MgrBLL::setAddFId(ui->InfoT->text());
                ui->WaringL->setText("请在文本框中输入增添的信息");
                addInfoSta=NameSta;
                ui->InfoT->setText("");
        ...
        case NameSta:
            ...
            {
                MgrBLL::setAddFName(ui->InfoT->text());
                ui->WaringL->setText("请在文本框中输入增添的信息");
                addInfoSta=TypeSta;
                ui->InfoT->setText("");
        ...
        case TypeSta:
            ...
            {
                MgrBLL::setAddFType(ui->InfoT->text());
                ui->WaringL->setText("请在文本框中输入增添的信息");
                addInfoSta=AddrSta;
                ui->InfoT->setText("");
        ...
        case AddrSta:
            ...
            {
                MgrBLL::setAddFAddr(ui->InfoT->text());
                ui->WaringL->setText("请在文本框中输入增添的信息");
                addInfoSta=FeeSta;
                ui->InfoT->setText("");
        ...
        case FeeSta:
            ...
            {
                MgrBLL::setAddFFee(ui->InfoT->text().toFloat());
                ui->WaringL->setText("请在文本框中输入增添的信息");
                addInfoSta=ExitSta;
                ui->InfoT->setText("");
                ui->WaringL->setText("");
        ...
        ui->dataLV->setModel(MgrBLL::getFiledByALL());
    ...
    else if(finshModel==Search)
    {
        ...
        {
            ui->dataLV->setModel(MgrBLL::getFieldByName(ui->InfoT->text()));
            ui->InfoT->setText("");
    ...
    else if(finshModel==Modfiy)
    {
        ...
        QString Id=ui->dataLV->model()->index(modRow,0).data().toString();
        QString Name=ui->dataLV->model()->index(modRow,1).data().toString();
        QString Type=ui->dataLV->model()->index(modRow,2).data().toString();
        QString Addr=ui->dataLV->model()->index(modRow,3).data().toString();
        float Fee=ui->dataLV->model()->index(modRow,4).data().toFloat();
        ...
        else
        {
            ...
            case 3:
                ...
                break;
            case 4:
                Fee=ui->InfoT->text().toFloat();
                break;
            default:
                break;
            }
            if(!MgrBLL::modifyField(Id,Name,Type,Addr,Fee))
            {
    ...
    else if(finshModel==Delete)
    {
        ...
        {
            if(!MgrBLL::deleteFieldById(ui->dataLV->model()->index(delRow,0).data().toString()))
            {
...
void MgrWindow::setSta()
{
    ...
    else if(sta==MgrInfoSta)
    {
        ...
        ui->dataLV->close();
        ui->WaringL->close();
        ui->IdL->setText("编号："+MgrBLL::getThisId());
        ui->NameL->setText("姓名："+MgrBLL::getThisName());
        ui->TeleL->setText("电话："+MgrBLL::getThisTele());
        ui->FeildL->setText("管理场地："+MgrBLL::getThisFName());
        ui->IdL->show();
        ui->NameL->show();
        ui->TeleL->show();
        ui->FeildL->show();
