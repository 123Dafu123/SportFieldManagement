...
void MgrWindow::on_FinshB_clicked()
{
    if(finshModel==Add)
    {
        ...
        case FeeSta:
            ...
            else
            {
                ...
                ui->InfoT->close();
                ui->FinshB->close();
                if(!DB.addField(feildinfo.Id,feildinfo.Name,feildinfo.Type,feildinfo.Addr,feildinfo.Fee))
                {
    ...
    else if(finshModel==Modfiy)
    {
        ...
        else
        {
            ...
            if(!DB.modfiyField(modFId,feildinfo.Name,feildinfo.Type,feildinfo.Addr,feildinfo.Fee))
            {
    ...
    else if(finshModel==Delete)
    {
        ...
        {
            if(!DB.deleteField(ui->dataLV->model()->index(delRow,0).data().toString()))
            {
