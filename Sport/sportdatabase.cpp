...
SportDataBase::SportDataBase()
{
    ...
    SportDB.setDatabaseName("SportFeild");
...
{
    QSqlQuery querry;
    querry.prepare("select * from MBRLogInfo where MBRId=:Id and MBRPswd=:Pswd");
...
{
    QSqlQuery querry1,querry2;

    querry1.prepare("insert into Member(MBRId,MBRName,MBRTele) values(:Id,:Name,:Tele)");
    querry1.bindValue(":Id",Id);
    querry1.bindValue(":Name",Name);
    querry1.bindValue(":Tele",Tele);

    querry2.prepare("insert into MBRLogInfo values(:Id,:Pswd)");
    querry2.bindValue(":Id",Id);
    querry2.bindValue(":Pswd",Pswd);

    return querry1.exec()&&querry2.exec();
}
...
bool SportDataBase::AddMgr(QString MGRId,QString MGRName,QString MGRTele,QString FName,QString SAId)
{
    ...
    QString FId;
    query1.prepare("select FId from Feild where Fname=:FName");
...
{
    QSqlQuery query;
    query.prepare("select MBRPswd from MBRLogInfo where MBRId=:MBRId");
...
bool SportDataBase::orderFeild(QString MBRId, QString FId)
{
    QSqlQuery query;
...
float SportDataBase::getOrderFFee(QString MBRId)
{
    ...
    query1.prepare("select FFee from Feild where FId=:FId");
...
{
    QSqlQueryModel *model=new QSqlQueryModel;
    model->setQuery("select * from Feild");
    model->setHeaderData(0,Qt::Horizontal,"场地编号");
    model->setHeaderData(1,Qt::Horizontal,"场地名称");
    model->setHeaderData(2,Qt::Horizontal,"场地类型");
    model->setHeaderData(3,Qt::Horizontal,"场地地点");
...
bool SportDataBase::addFeild(QString FId, QString FName, QString FType, QString FAddr, float FFee)
{
    QSqlQuery query;
    query.prepare("insert into Feild values(:FId,:FName,:FType,:FAddr,:FFee)");
    query.bindValue(":FId",FId);
    query.bindValue(":FName",FName);
...
{
    QSqlQuery query;
    query.prepare("select * from Feild where Fname=:FName");
    query.bindValue(":FName",FName);
    query.exec();
    QSqlQueryModel *model=new QSqlQueryModel;
    model->setQuery(query);
...
bool SportDataBase::modfiyFeild(QString modId,QString FName, QString FType, QString FAddr, float FFee)
{
    QSqlQuery query;
    query.prepare("update Feild set Fname=:FName,FType=:FType,FAddr=:FAddr,FFee=:FFee where FId=:modId");
    query.bindValue(":FName",FName);
    query.bindValue(":FType",FType);
    query.bindValue(":FAddr",FAddr);
    query.bindValue(":FFee",FFee);
...
bool SportDataBase::deleteFeild(QString FId)
{
    QSqlQuery query;
    query.prepare("delete Feild where FId=:FId");
    query.bindValue(":FId",FId);
    return query.exec();
}
